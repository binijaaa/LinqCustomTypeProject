...
namespace LinqCustomTypeProject 
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var listCars = from car in cars 
                           select car.Name; //get name(real value) of each object

            foreach (var element in listCars)
            {
            ...
            var listUnitsSold = from car in cars 
                                select car.UnitsSold; 

            foreach (var element in listUnitsSold)
            {
                Console.WriteLine(element);
            }
            ...
            var listWithExpression = from car in cars
                                     where car.UnitsSold > 60000
                                     orderby car.UnitsSold descending
                                     select car;

            StringBuilder stringBuilder = new StringBuilder();
            foreach (var car in listWithExpression)
            {
